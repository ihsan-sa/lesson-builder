...
**colour** — palette adherence to the Lumen light system (accent `#C96442`, surface `#F4F1EB`, canvas `#FAF9F6`, axis/muted `#9C988F`, text `#3A3833`, border `#E8E4DC`; secondary series blue `#3E6C8F`, red `#B14B3F`, green `#4F7A52`, purple `#7A5B86`, orange `#C08A3E`; plus `#0b0b0c` for manim video frames, its required background, with `#c8a45a`/`#e6e6e6` on video; small tolerance for anti-aliasing); text legible against background; curves distinguishable from each other and the background; **no dark-field artwork embedded in the lesson body** — a lesson opens in the light theme, so a dark figure reads as a hole in the page; colour is not the only channel distinguishing >3 categories.
