...
| 12 | a thread turn is told apart from its parent's in `chat.log` | A thread shares its parent's `chatNum` and numbers its own turns from 1, so a parent turn and a thread turn can both be `msg=1` and in flight at once. The fake CLI's `HOLD<tag>` holds the parent open until the check releases it after the thread turn has run start to finish — a release, not a fixed delay, because under box load the thread's CLI once took longer to start than the delay did. Every per-turn line carries `threadId` for a thread's turn: a reader keyed on `(chatNum, threadId, msg)` sees each `CHAT_OK` close the entry its own `CHAT_START` opened, none evicted and none left open. Kept: the thread's `CHAT_OK` says `threadId=t12`; suppressed: the parent's says none. Fake CLI only. |
