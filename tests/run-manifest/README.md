...
| 7b | a per-lesson `consolidate` run whose `plan.approval` is `inherited`, with the consolidation plan's hash in `via` and no plan of its own | `render` writes `Approval: INHERITED from consolidation plan <that hash> at <ts>` — the hash comes from `via`, never from the run's own `plan.hash`, which is `null` here |
