...
EXCLUDED=(
  ...
  "phone-width --sweep|the fixture measures its own scaffolded lessons; this measures overlap and collapse over every lesson of a real built site|serve a build (bin/serve-dist.mjs --port 5301 --dist <site> --quiet), then PHONE_WIDTH_BROWSER=/usr/bin/google-chrome SITE_URL=http://127.0.0.1:5301/ node tests/phone-width/sweep.cjs"
  "phone-width --sweep-negative|the sweep's negative control: it needs a built site to copy a lesson out of, and a Chromium|SITE=<a built site> PHONE_WIDTH_BROWSER=/usr/bin/google-chrome tests/phone-width/sweep-negative.sh"
