...
| 3 | replacing a graph component | the file afterwards is byte-for-byte the old file with that one line range swapped — recomputed here from the range and the replacement, not read from the receipt — every other line is unchanged and in order, the result parses, the receipt names the range and the bytes each way, its `declarations` report every other top-level declaration verified byte-identical with nothing changed, removed or added, its `bytes_on_disk` matches, and no `.part` file is left |
| 3c | a scratch file that brings more than the component it replaces | a helper alongside the component lands and the receipt names it under `declarations.added`; a scratch file that redeclares an existing binding (`axisTicks`) exits 4 naming it, leaves the lesson byte-identical and writes no `.part` — the declaration check is the only thing that sees this, because it compares declaration source by name across two parses rather than the offsets the splice used |
...
| 5 | removing a graph, its call site, its params key and its schema key in one pass | all four are gone with no run of blank lines where they were, the surviving `<LiveGraph>` wrapper keeps its call site and no empty wrapper is left, the two key lists still agree, exactly the lines the receipt named are gone with no other line moved, and the receipt verifies every top-level declaration outside the four it may touch |
