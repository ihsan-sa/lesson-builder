...
| 2b | a first run that ended with a `fail` on one medium and an `issue` on another | with nothing touched, both are reviewed again for the reason `prior verdict was <v>` — an unclean verdict stands on findings that are still open, so it is never carried forward — while the clean verdicts on the very same media are reused; the gate refuses until those two are judged again and passes once they are |
...
Case 2b is the one that keeps coverage honest in the other direction: reuse is proof that nothing
needs doing, so only a `pass` is ever reused.
