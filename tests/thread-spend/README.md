...
| 9 | the main chat's spend across a reload (landing review, 2026-09-23: threads and the transcript survived a reload, the tab's own total did not) | `lessonSessions.js`'s `spendKey` is scoped like `msgsKey`/`reinfKey`; the exact save/restore round trip Chatbot.jsx runs (write only when `tab.spend` is a number, read back only when the parsed value is a finite number) keeps a real total, writes nothing for a tab with no cost yet, and ignores garbage already in `sessionStorage` rather than coercing it; the save effect writes it beside `reinfKey`, and `resumeSessionIntoTab` only merges `spend` into the tab when its restore actually produced a number |
