...
The presentation layer is the Claude Design **Lumen** shell (`@core/ui/LessonShell.jsx`): a 66px top bar, a 262px collapsible contents rail with per-topic section outline and scroll-spy, a 680px article column, and a tutor panel that docks to the side or bottom, floats in an in-app window, or detaches into a real browser window. Below 720px the same shell switches to its phone layout: an app bar and section strip, the contents and the tutor as bottom sheets, and a prev / next / "Ask the tutor" bar, with 44px tap targets and equations that scroll sideways instead of shrinking. Light and dark: the top bar carries a DARK/LIGHT button, the same switch the 39 pre-shell lessons have, and the shell swaps the palette on its root and on the detached tutor window. Lessons never hand-roll chrome; they supply `TOPICS` plus the active topic's body and let the shell frame it.
...
**Explanations are planned, not improvised.** Content architecture (backward design, prerequisite ordering) and interaction architecture (the tutor's retrieval-first policy) were always specified; the discourse layer — how each explanation unfolds — is specified in `references/teaching-communication.md` and enforced end to end: Phase 1 recovers the relations between claims (a missing relation a novice must infer is a gap), Phase 2 emits a `teaching_arc` per topic (central question, entry state, purposed moves, example functions, exit model, exit evidence — and rejects content lists by the reorder test), Phase 3 authors prose against it in a fixed assembly order (shortest explanation that leaves no necessary inference unstated; one controlling claim per paragraph; representation matched to structure; analogies off by default; no seductive detail), Phase 4's `content-review-agent` runs an exhaustive accuracy pass and a calibrated discourse pass (kinds, 0–3 rubric with gates, arc check), and the tutor prompt carries the teaching spec's style block as `TEACHING_COMMUNICATION` (the v4 text plus the 2026-09 Stage 1 additions, listed in that file) with seven response modes and budgets. "Prefer equations over prose" is not a rule anywhere in this skill — prose is the medium for causal reasoning; a figure is produced when the governing relation is a shape, a structure or a change across stages, and media never repair discourse.
...
Acceptance criterion: after bootstrap + new-mode Phases 0-4, a skeleton lesson must reach `18/18 passed` on `test_lesson.cjs`, render KaTeX, show the chatbot bubble in dev, respect the Ctrl+Click context gate, and render `<DesmosGraph/>` (when `VITE_DESMOS_KEY` is set). See `references/bootstrap.md` for the full checklist.
...
<workspace_root>/
  ...
  <course>/claude_lessons/<slug>/
    ...
    test_lesson.cjs             18-test QA suite
    index.html
...
**Chat runtime facts** authors and reviewers should know: the canonical tutoring PEDAGOGY POLICY (retrieval-first, least-help-first hint ladder, task-level feedback, question discipline, adaptive explicitness, feedback anatomy) and the TEACHING COMMUNICATION layer (the spec's style block: priority order, seven response modes with budgets, prose/format/tone rules, analogy policy, turn control, anti-sycophancy; plus few-shot exemplars) are injected by `@core/chat/buildSystemPrompt.js` — lessons do NOT paste either into `LESSON_CONTEXT`, and legacy lessons that pasted the policy are marker-detected and not double-injected. The system prompt must stay under the proxy's 28 000-char argv threshold (`SYSTEM_ARGV_CEILING` in `@core/constants/promptBudget.js`; the core is ~25.5k before `LESSON_CONTEXT`, headroom ~0.2k on the largest of 48 lessons), so keep `LESSON_CONTEXT` to course specifics. Each lesson's `test_lesson.cjs` T18 measures the assembled prompt and fails over the ceiling. The chat panel renders in dev, and in a build that sets `VITE_TUTOR=1` — `TUTOR_ENABLED` in `@core/constants/build.js` gates it (and all `/session` fetches) out of a plain `vite build`, which is what the static deploys that have no proxy run. The same module resolves every chat URL against the build's own base, so a lesson built with `--base="/<code>/<slug>/"` calls `/<code>/<slug>/chat` and reaches its own backend rather than the site root; `tests/hosted-build` asserts both bundles. The chat opens on `DEFAULT_MODEL`/`DEFAULT_EFFORT` from `constants/models.js` (the `default: true` entry; keyboard-shortcut chars must be unique and avoid j/g). The proxy passes the selected model name to the `claude` CLI unchanged, so a specific version pick runs that exact version. `<Chatbot>` accepts an optional `institution` prop (e.g. `institution="University X"`) that appears in the tutor system prompt; omit it and no institution is mentioned.
