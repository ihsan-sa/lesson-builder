...
`AskUserQuestion` must not fire in the last two — the dialog would render where nobody is looking. Gates with a documented safe default (gitignore override → no override, dirty tree → continue and report, orphans → keep) take it and report rather than blocking; only decisions that cannot be defaulted block. Details: `SKILL.md` § Session modes and gates.
