...
- Threads have their own attachments (`+`, paste, drag-drop) and can be stopped mid-reply — a thread's Stop reaches only that thread. The tutor may draw a graph or cite sources inside a thread; graph edits, lesson-augmentation suggestions and commit offers only appear on main-transcript messages.
- **A thread is its own session**, forked from the main conversation when you first send in it: the main tutor never sees what you explore there. The `⤴` in the thread header folds it back — one summary message you read first, which then rides your next main message. That is the only way a thread reaches the main conversation.
- The chat panel renders in dev, and in a build that sets `VITE_TUTOR=1` (`TUTOR_ENABLED` in `_lesson-core/constants/build.js`). A plain `vite build` therefore ships without it, which is what a static host — no proxy to answer a chat call — should get.
