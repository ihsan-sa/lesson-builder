...
// built for. That sheet is chat/shell.css.js and LessonShell injects it; the
// two lessons that render inside the shell get it over this one. Nothing here
// is shell-specific, and no lesson had to be edited to tell them apart.
//
// Every var(--x) used here is declared in both theme blocks below; tests/
// css-vars-defined checks that, because the forward-ported rules arrived
// referencing tokens this sheet never defined and painted with unset values.
export const STYLES = `
...
:root, .theme-dark {
  ...
  /* Tokens the forward-ported rules at the foot of this sheet ask for, mapped
     onto this sheet's palette so they follow the theme. Both theme blocks must
     declare them: an alias resolves where it is declared, so declaring these
     only in :root would hand .theme-light the dark values. */
...
.chat-header-spend { font-size: 11px; color: var(--text-dim); font-family: 'IBM Plex Mono', monospace; opacity: 0.75; margin-left: 6px; }
...
.thread-spend { font-size: 11px; color: var(--text-dim); font-family: 'IBM Plex Mono', monospace; flex-shrink: 0; opacity: 0.75; }
...
/* ---- forward-ported: rules the classic sheet predates ---- */

/* The one declaration below that lands on a class the sheet above already
   styles, and it is deliberate. .eq-block above is position: static; .eq-side
   right below is position: absolute and anchors to its nearest positioned
   ancestor. With no anchor the side rail escapes to the initial containing
   block, and every Explain button in a lesson stacks at one point outside the
   content column and below the fold -- the button is still in the DOM and
   still works, but nobody can reach it. This anchors the rail; it repaints
   nothing, so the panel keeps the classic beige fill and gold left rule.
   Not forward-ported with it: .eq-block[data-latex] { padding-right: 92px }
   from the newer sheet, which reserves room for the rail. That padding is
   asymmetric and would shift the centred equation off the panel's middle,
   which is a look change. Nothing reserves room for the rail, so at a narrow
   width it lands on top of the equation -- see the media query below. */
.eq-block { position: relative; }
...
/* Anchoring the rail (above) is what makes it overlap a wide equation, because
   the panel reserves no room for it. There is width to spare beside a centred
   equation on a desktop; at a phone width there is none, and the pill lands on
   the equation it belongs to. So below this width the rail stops floating and
   flows under the equation instead: nothing is covered, and the panel grows by
   the height of one pill. 520px is the breakpoint the sheet already uses. */
