...
- CSS in `_lesson-core/chat/chat.css.js`, and in `chat/shell.css.js` for a LessonShell lesson, gates the pointer cursor and hover-outline on that class — transitions stay outside the gate so the fade-in feels smooth the moment Ctrl is pressed.
...
- [ ] Branch name `lesson-update/<slug>-YYYYMMDD` does not already exist locally. If it does, Phase 3's `run-manifest.cjs branch` takes the first free `-a`…`-z` suffix and records the name it created — the collision handling is deterministic there rather than chosen per session, so the Phase 5 merge target is unambiguous. Surface the collision at mode confirmation all the same, in case the user meant to finish the earlier run instead of starting another.
