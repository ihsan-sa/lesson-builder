...
Cross-reference: `~/.claude/skills/lesson-builder/SKILL.md` holds the skill-level mode-detection summary and phase-shell layout. This doc expands only the update-specific concepts.
...
**Lesson references** (from SKILL.md): a workspace root is derived from the current working directory (or asked of the user in Phase 0). Candidates are resolved via `Glob <workspace_root>/*/claude_lessons/*/` plus any path containing `claude_lessons` that the user pastes directly.
...
- **Example**: an existing SVG graph whose equation and axis scales are already correct stays exactly as is.
...
- **Example**: a graph function keeps its name (so every call site in the lesson's `TOPICS` content remains valid) but its y-axis scale is recomputed by graphics-agent using a `refine_brief`.
...
- **Example**: a static SVG that the topic now wants to present as a temporal/interactive exploration — `interactive-demo-agent` builds a new `<InteractiveDemo>` primitive to replace it. The old function and its call site are excised; the new primitive and new call site are inserted; `DEFAULT_GRAPH_PARAMS` / `GRAPH_SCHEMA` updated accordingly.
- **Specialist assigned**: the **destination-type** specialist (whatever kind the new medium is, not the old kind).

### remove

The concept was removed from the lesson, the user flagged it for cut, or it violates the low-value patterns from `jsx-lesson/SKILL.md:170-174`. Component definition, call site, `DEFAULT_GRAPH_PARAMS` entry, and `GRAPH_SCHEMA` entry all excised.
- **Example**: a graph whose underlying concept has been cut from the lesson — its definition, call site, and param entries all disappear in Phase 3 assembly.
...
- **Example**: the topic needs a temporal visualization and no existing medium supplies it; `manim-agent` builds a fresh animation and the splice inserts the new `<video src>` reference in the relevant topic content function.
...
### Branch name format
`lesson-update/<slug>-YYYYMMDD`. One branch per update run. Created in Phase 3 (not earlier), never on main.
...
- **Non-`@core` lessons**: any lesson whose JSX still inlines chat code rather than importing from `@core` is an update-mode no-go by default. Detection: single Grep for `from "@core"` (or the relative path shim `from "../../../_lesson-core"`) in `src/<slug>.jsx`. On miss, surface "this lesson needs migration first; opt into update-without-migration, abort, or switch to new mode?" during mode confirmation. Narrow opt-in bypass exists only if the user explicitly picks it.
...
- **Orphan assets**: files present under `<lesson_root>/public/images/`, `<lesson_root>/public/videos/`, or `<lesson_root>/*.py` with no JSX reference. Inventory generation Globs both the JSX and the filesystem; orphans get flagged in the Phase 2 change-list under a dedicated `ORPHAN ASSETS` subsection with a `keep | remove` pre-verdict per file for the user to confirm or override at the approval gate. Phase 3 step 4.9 reads the approved verdicts and deletes `remove`-marked files from disk during the orphan-asset-cleanup drift repair. Phase 5's final report surfaces removed/kept counts.
