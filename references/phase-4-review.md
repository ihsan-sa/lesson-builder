...
**Two named passes (both modes).** The same spawn runs an **accuracy pass** (exhaustive — every correctness and alignment problem, uncertain ones included) and a **discourse pass** (calibrated — a finding is reported only when it names the learner cost and the violated contract). The discourse pass walks every topic's prose for the discourse issue kinds — `sequence`, `missing_prerequisite`, `missing_inference`, `explanation`, `paragraph_unity`, `cohesion`, `representation_mismatch`, `redundancy`, `example_function`, `analogy`, `seductive_detail`, `hedging`, `filler`, `register`, `terminology` — and runs the `arc` check on every topic that has a `teaching_arc`: dependency structure, `central_question`, `exit_model`, and `exit_evidence` preserved (reordering or merging moves is not a violation). It answers the gate question per topic (*could the intended learner reconstruct why every major step follows, without inventing an unstated intermediate idea?*) in `topic_gates`, scores the **0–3 rubric** per topic (purpose, macro-order, explanatory adequacy, paragraph unity, cohesion, inference burden, terminology, format fit, example design, economy, adaptation, closure) in `rubric`, and returns per-topic `diagnostics` (lints: bullet density, causal-connector bullets, stem-less bullets, orderless numbered lists, thin headings, nested lists, analogies per 1000 words, hedge/filler hits, undefined symbols, paragraph-length distribution, repeated summaries, exit checks with new definitions). Lints route to review and never auto-fail. The kinds table, severity calibration, and pass procedure are canonical in `agents/content-review-agent.md`; the rules they enforce are in `references/teaching-communication.md`; the rubric text is `evals/teaching/rubric.md`. Kind and severity are orthogonal — the reviewer rates each instance `minor` or `major`; discourse kinds never rate `blocker`.

**Rubric gates** main Claude applies when compiling findings: any **0 on macro-order, adequacy, inference burden, or terminology** on a topic → one `major` (`[rubric] …`) for that topic; **average < 2.0** → the topic is a revise (fix-loop item, not a forward-as-known-issue); **economy = 3 with adequacy ≤ 1** → never ship as-is — the topic was made short by deleting reasoning; treat as a `major` on adequacy regardless of the other scores.
...
**Reviewer calibration** is a maintenance step, not a per-run one: after any change to `agents/content-review-agent.md` or `references/teaching-communication.md`, run the reviewer blind against `evals/teaching/lesson-fragments/` (12 reviewer fixtures) and the fixture rows of `evals/teaching/tutor-cases.jsonl` (12 tutor fixtures) per `evals/teaching/README.md`, grading against `evals/teaching/expected-failures.json` (never shown to the reviewer), and require all twenty-four seeded failures flagged.
...
- **Arc preserved and the gate question answered.** For every topic with a `teaching_arc`, the reviewer's `arc` check passes (dependencies, central question, exit model, exit evidence intact) and `topic_gates[].reconstructible` is true. A false gate answer names the unstated step; the fix is to state it, not to delete the surrounding reasoning.
- **Discourse floor.** No `major` discourse finding left open on a new-mode topic or a rewritten update topic: no consequence before its rule, no inference or mechanism the stated audience cannot supply, no unmapped or misleading analogy, no seductive-detail passage, no causal argument fragmented into bullets, no undefined symbol carrying the point, no paragraph without a controlling claim, no rubric gate tripped. `minor` discourse findings (a stray hedge, a small redundancy, one weak transition) are logged and fixed when the fix is local; they never block.
