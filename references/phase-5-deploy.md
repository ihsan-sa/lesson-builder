...
**Filing it.** The box's document register (`cc-docs`) numbers, stamps and files a finished PDF when the build's environment carries `DOC_PROJECT` and `DOC_TITLE`; `pdf-material-builder` does the filing, this step only sets the variables, and only on the finished build (never on a draft or a retry that will be rebuilt). Set `DOC_PROJECT=<course code>` (the `<course>` directory's code), `DOC_KIND=course` and `DOC_TITLE=<the lesson title>`. A lesson built for a member workspace sets `DOC_KIND=member` and `DOC_MEMBER=<that member's handle>` instead. `DOC_NO_STAMP=1` is optional and is never set here. With `DOC_PROJECT` unset, or `cc-docs` not on PATH, the build runs exactly as before, so a machine without the register loses nothing; a filing failure is a `Companion PDF: FAILED` note like any other, not a gate.
...
**Linking it from the lesson.** Once the companion has built, the lesson links it, with the href from `companionHref("<the .pdf's basename>")` (exported by `@core`) and never a literal path. A shelled lesson adds `{ label: "Printable companion", href: companionHref("…") }` to `LessonShell`'s `refs`; a classic lesson puts an `<a href={companionHref("…")} target="_blank" rel="noreferrer">` in its header. Then re-run Step 1's build, so that the build Step 2 ships is one that passed with the link in it; if that build fails, take the link out again and log it in the `Companion PDF: FAILED` note, since a lesson that built without the link is still ready to deploy. `companionHref` returns `VITE_COMPANION_HREF` when the build command sets it to an `https://` or site-absolute link, which the lessons workspace's `build-all.sh` can set per lesson to the lesson's library link (`https://library.ihsan.cc/l/<token>`) so the PDF can be repointed without a rebuild. Unset, it returns `<base>/<basename>`, the copy `build-all.sh` publishes beside the lesson's build at `/<prefix>/<slug>/`. Any other value is named in the console and falls back to that copy. The value is per lesson, so it never goes in the workspace-root `.env.local`; `tests/hosted-build` covers both sides.

**When it publishes.** Step 1's `build-all.sh` ran before this step, so this run's local `dist/` never contains the companion; the copy that publishes it is the host's rebuild after the push, or the user's next local build. A workspace whose `.gitignore` has no negation for `*/claude_lessons/*/*_companion.{tex,pdf}` never tracks it at all (`pdf-material-builder/docs/integration.md` §1), and Step 2's staging check below is what says so out loud.
