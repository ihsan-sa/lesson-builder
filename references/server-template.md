...
| T9 | Renders in LessonShell | File renders `<LessonShell>` and applies a `theme-` class (the loading gate's ``className={`theme-${theme}`}``; the shell applies `theme-light` / `theme-dark` thereafter). Lumen tokens come from `@core` CSS, which declares both palettes. |
