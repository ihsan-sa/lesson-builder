...
A missing `_lesson-core/` is the main failure mode this bootstrap fixes. If the directory is present, run the core-version check below, then proceed to `references/phase-0-scoping.md`.

## Core-version check (existing workspaces)

A workspace's `_lesson-core/` — the tutor's model list included — must never lag the skill. At the start of every build, new or update, and whenever anything reports drift, run:

```
"$SKILL/scripts/core-refresh.sh" refresh <workspace_root>
```

It is not a decision and never a question for the user, in any session mode: the smoke test decides, not a person. Drift is a file the skill ships under `references/bootstrap/_lesson-core/` that is missing from the workspace's core or differs from it. On drift the script copies the whole core aside, copies every shipped file over its workspace copy, runs `npm install` there, confirms no drift is left and runs its smoke test: the lesson template scaffolded into a throwaway course with a body that imports all of `@core`, built with `vite build`, and its proxy started and asked for `/sessions`. No model call.

- **What it leaves alone.** Files the skill does not ship (a workspace's own additions), `node_modules/` and `package-lock.json`. A local edit to a *shipped* file is overwritten; a workspace that needs a core change makes it in a file of its own or in the skill.
- **Exit 0** — current, or refreshed and the smoke test passed. Continue.
- **Exit 3** — the refresh or its smoke test failed, and the previous core was put back byte for byte (its banner reads `CORE REFRESH ROLLED BACK`). Say so in the first line of the next message to the user and log it under Phase 0 notes; never carry on quietly. Then read the restored core: `server/viteLessonProxy.js` missing → halt the build, because the template's `vite.config.js` imports it and a new lesson cannot start. `PEDAGOGY_POLICY` missing from `chat/buildSystemPrompt.js` → continue, but Phase 3 embeds the PEDAGOGY POLICY text (from the payload's `buildSystemPrompt.js` export) verbatim into the new lesson's `LESSON_CONTEXT`, since the old core injects none. Otherwise continue on the old core.
