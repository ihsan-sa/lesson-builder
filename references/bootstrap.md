...
The skill ships the payload at `references/bootstrap/`. Everything is verbatim from a working 2A workspace; it is the single source of truth.
...
references/bootstrap/
  _lesson-core/              Drop-in copy of the shared module imported via @core
    ...
    ui/                      Eq, primitives (P, Section, KeyConcept, CollapsibleBlock,
                             RefImg), primitives-interactive (Slider, Toggle, …),
                             DesmosGraph
    ...
    server/proxy.js          Canonical Express proxy (imported via shim from
                             every lesson's server/proxy.js)
  lesson-template/           Skeleton lesson project (used at new-mode Phase 3)
    ...
    vite.config.js           @core alias + server.fs.allow + proxy routes
...
Create these three at `<workspace_root>/` if missing. Do **not** overwrite existing files without confirming with the user first; the workspace may already have a working `.gitignore` or `.env.local`.
...
| `$SKILL/references/bootstrap/workspace-root/env.local.example` | `<workspace_root>/.env.local` | Workspace-root only — Vite walks upward to find it; do NOT copy one per lesson. Fill `VITE_DESMOS_KEY` (obtain from https://www.desmos.com/api). Never commit. |
...
2. Replace placeholders:
   - `__SLUG__` → the lesson slug (dash form) in `package.json` `"name"` and `test` script.
   - `__SLUG_SNAKE__` → snake form in `package.json` `test` script and `src/main.jsx`.
   - `__COURSE_CODE__` + `__LESSON_TITLE__` → display-friendly strings in `index.html`.
