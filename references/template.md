...
- **The shell owns the chrome.** Do not hand-roll a header, a tab bar, a footer or a content wrapper — `LessonShell` renders the top bar, the contents rail (with per-topic section outline and scroll-spy), the article column, and the tutor dock — and below 720px the phone layout of all four, so a lesson needs no media queries of its own for its frame. The lesson supplies `TOPICS` and the active topic's body as children. `LessonShell` also injects its own sheet, `SHELL_STYLES` from `@core/chat/shell.css.js`, so a lesson needs no `<style>` tag outside the KaTeX loading gate.
